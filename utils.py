...
def suggest_optimal_model(df_info):
    ...
    prompt = (
        ...
        "Учитывая, что главными требованиями являются высокая точность предсказаний и быстрая работа модели, "
        "какая модель оптимально подойдет для этих данных? Дай ответ одним словом с кратким пояснением."
    )
    return get_chatgpt_response(prompt)
