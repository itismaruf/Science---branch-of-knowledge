...
from utils import (
    ...
    plot_data_visualizations, train_model, plot_predictions, default_cleaning, remove_outliers_iqr, suggest_optimal_model
)
                ...
                if st.button("✨ Получить интерпретацию"):
                    if user_description.strip():
                        ...
                        prompt = (
                            ...
                            "Проанализируй структуру и содержание данных. Кратко опиши, какие ключевые признаки присутствуют, какие могут быть важны для анализа или принятия решений. "
                            "Выдели интересные закономерности, возможные проблемы или пробелы в данных. "
                            "Смотри данных — в зависимости от их тематики (например, если это об образовании, финансах, здравоохранении, городской среде и т.д.). "
                            "Формулируй ответ ясно и конкретно, без излишней общности. (Обрашай внимание на короткость и ясность чтобы читать не лень было!!!)"
                        )
...
# === Автообработка данных ===
    ...
    if "df" in st.session_state:
        ...
        # Кнопка для удаления выбросов, вызов функции из utils.py
        if st.button("🗑️ Удалить выбросы (IQR)"):
            with st.spinner("Удаление выбросов по методу IQR..."):
                # Вызываем функцию из utils.py, которая удаляет выбросы
                df, removed_rows = remove_outliers_iqr(df)
                st.session_state["df"] = df
            st.success("✅ Выбросы удалены.")
            if removed_rows > 0:
                st.info(f"Удалено {removed_rows} строк с выбросами по методу IQR. Это помогает устранить аномалии и повысить качество модели.")
            else:
                st.info("Выбросов не обнаружено или они не требуют удаления.")
...
# === Предсказание модели ===
    ...
    with st.expander("ℹ️ Описание раздела:"):
        ...
        основываясь на характеристиках данных (а также вашего описания целей из раздела «Загрузка данных», если оно задано). 
        Затем выбирайте целевую переменную и модель для обучения.
    ...
    if "df" in st.session_state:
        ...
        # Рекомендация целевой переменной
        if st.button("💡 Рекомендовать целевую переменную для предсказания"):
            ...
            st.info(f" Рекомендуемая целевая переменная: {recommended_target}")

        # Новая кнопка для рекомендации оптимальной модели,
        # если требуется (аналогичная логике прошлой рекомендации)
        if st.button("📌 Рекомендовать оптимальную модель для предсказания"):
            df_info = st.session_state.get("data_summary", "Сводка данных отсутствует")
            recommended_model = suggest_optimal_model(df_info)
            st.info(f"Рекомендуемая модель: {recommended_model}")
        
        # Стандартный выбор целевой переменной и модели
        ...
        if model_type and target:
            try:
                ...
                if metrics is not None:
                    ...
                    st.error(
                        "Возникла ошибка при обучении модели. "
                        "Проверьте, что данные корректны (например, все пропуски обработаны, "
                        "выбранная целевая переменная имеет как минимум два класса, и данные соответствуют требованиям модели)."
                    )
            except Exception:
                st.error(
                    "Произошла ошибка во время обучения модели. "
                    "Пожалуйста, убедитесь, что данные подготовлены корректно (используйте функцию умной очистки) и попробуйте снова."
                )
        else:
            st.info("Выберите целевую переменную и модель для обучения.")
    else:
        st.warning("Сначала загрузите данные")
...
# === Разъяснение результатов ===
    ...
    # Список предложенных вопросов (radio позволяет выбрать один вариант)
    selected_question = st.radio("Выберите вопрос:", suggested_questions)
    
    # Кнопка для заполнения текстового поля выбранным вопросом
    if st.button("Заполнить поле выбранным вопросом"):
        st.session_state["chosen_question"] = selected_question

    # Текстовое поле для ввода/редактирования вопроса.
    # Если пользователь уже выбрал вопрос, он будет подставлен по умолчанию.
    default_text = st.session_state.get("chosen_question", "")
    ...
    # Кнопка для отправки запроса с итоговым текстом вопроса
    if st.button("Отправить запрос"):
        ...
        if final_question:
            ...
            st.warning("Пожалуйста, выберите или введите вопрос перед отправкой.")
